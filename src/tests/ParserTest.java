...
public class ParserTest
{
    ...
    public void testBinaryOps ()
    {
        ...
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(mod (mul 6 0.5) (plus 1 1))").evaluate(0,0);
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(exp (mul 2 1) (plus -1 1))").evaluate(0,0);
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(neg (mul -1 1))").evaluate(0,0);
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(color .75 .32 (mul -1 1))").evaluate(0,0);
        assertTrue(new RGBColor(.75, .32, -1).equals(actual));
        actual = myParser.makeExpression("(floor 1.2)").evaluate(0,0);
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(ceil 0.5)").evaluate(0,0);
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(abs -1)").evaluate(0,0);
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("(wrap 1.75)").evaluate(0,0);
        assertTrue(new RGBColor(-0.25).equals(actual));
        actual = myParser.makeExpression("(wrap 4.75)").evaluate(0,0);
        assertTrue(new RGBColor(0.75).equals(actual));
        actual = myParser.makeExpression("(wrap 2.75)").evaluate(0,0);
        assertTrue(new RGBColor(0.75).equals(actual));
        actual = myParser.makeExpression("(wrap -1.75)").evaluate(0,0);
        assertTrue(new RGBColor(0.25).equals(actual));
        actual = myParser.makeExpression("(wrap -2.75)").evaluate(0,0);
